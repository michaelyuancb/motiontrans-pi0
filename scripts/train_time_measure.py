...
def train_step(
    ...
    model.train()
    print(f"Model_State: {time.time() - st}")
    ...
    print(f"Train_RNG: {time.time() - st}")

    st = time.time()
    # Filter out frozen params.
    diff_state = nnx.DiffState(0, config.trainable_filter)
    print(f"Diff_State: {time.time() - st}")
    st = time.time()
    loss, grads = nnx.value_and_grad(loss_fn, argnums=diff_state)(model, train_rng, observation, actions)
    print(f"Calc-Loss: {time.time() - st}")

    st = time.time()
    params = state.params.filter(config.trainable_filter)
    print(f"Filter_PARAM: {time.time() - st}")
    st = time.time()
    updates, new_opt_state = state.tx.update(grads, state.opt_state, params)
    new_params = optax.apply_updates(params, updates)
    print(f"Update Param: {time.time() - st}")

    st = time.time()
    # Update the model in place and return the new full state.
    nnx.update(model, new_params)
    new_params = nnx.state(model)
    print(f"Update Model: {time.time() - st}")

    print(f"EMA_OPEN: {state.ema_decay}")
    st = time.time()
    ...
    print(f"EMA_STATE: {time.time() - st}")
    ...
    print(f"Kernel Param: {time.time() - st}")
    st = time.time()
    info = {
        "loss": loss,
    ...
    print(f"Global-Info: {time.time() - st}")


    print(f"Overall Training Time: {time.time() - st_begin}")
...
def main(config: _config.TrainConfig):
    ...
    infos = []
    for step in pbar:
        st_with_shard = time.time()
        with sharding.set_mesh(mesh):
            st = time.time()
            train_state, info = ptrain_step(train_rng, train_state, batch)
            print(f"Training_Step: {time.time() - st}")
        print(f"Training_Step with Shard_MESH: {time.time() - st_with_shard}")
        # infos.append(info)
        ...
        st = time.time()
        batch = next(data_iter)
        print(f"Load Data: {time.time() - st}")
...
if __name__ == "__main__":
    ...
    # if "nairuiqian" in hostname:
    #     name = "nairuiqian"
    #     password = "nrq@sqz2024"
    # else:
    #     exit()
